/* TOP FRAGGERS — config store */

const STORAGE_KEY = "top-fraggers-config";
const CHANNEL_NAME = "top-fraggers-sync";
...
export async function uploadBackground(file) {
  ...
  const res = await fetch(`${getApiBase()}/top-fraggers/upload-background`, { method: "POST", body: fd });
  if (!res.ok) {
...
export function ensureCards(config) {
  const count = Math.max(3, Math.min(5, Number(config.cardCount) || CARD_COUNT));
  config.cardCount = count;

  if (!Array.isArray(config.cards)) config.cards = [];
  while (config.cards.length < count) {
    const i = config.cards.length + 1;
    config.cards.push({ name: `PLAYER ${i}`, finishes: 0, characterImageUrl: "" });
  }
  config.cards = config.cards.slice(0, count);
  ...
  if (!th.mediaTintTop || !th.mediaTintBottom) {
    ...
    if (!th.mediaTintTop) th.mediaTintTop = stops[0] || "#1a4d7a";
    if (!th.mediaTintBottom) th.mediaTintBottom = stops[1] || stops[0] || "#0a2840";
  }
  if (!th.mediaTint) {
  ...
  if (!config.header) {
    config.header = {
      ...
      rightLogoUrl: "",
      brandMain: "MEA",
      brandSub: "MUMBAI ESPORTS",
    };
  }
  if (!config.footer) config.footer = { text: "#MUMBAI LAN EVENT", color: "#e8c547", size: 36, show: true };
  if (config.dataSource !== "manual") config.dataSource = "live";
}
...
export async function saveConfig(config) {
  ...
  try {
    const res = await fetch(`${api}/top-fraggers/config`, {
      method: "POST",
...
export async function getConfig() {
  ...
  try {
    const res = await fetch(`${getApiBase()}/top-fraggers/config`);
    if (res.ok) {
    ...
    console.warn("[top-fraggers] server load:", e.message);
  }
