...
import { MAP_CATALOG, MAP_KEYS } from "./map-catalog.js";
...
function updateBgPreview(url, mediaType) {
  ...
  if (isVideo) {
    els.bgPreview.innerHTML = `<video src="${safe}" autoplay loop muted playsinline></video>`;
  } else {
    els.bgPreview.innerHTML = `<img src="${safe}" alt="Background preview" />`;
  }
}
...
function renderMatchTabs() {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (m.isWinner || m.showWwcd) btn.classList.add("is-wwcd");
...
function matchEditorHtml(i) {
  ...
  return `
      ...
      <div class="field-row">
        <div class="field"><label>Map</label>
          <select data-k="mapKey" data-i="${i}">
            ${MAP_KEYS.map((k) => `<option value="${k}" ${m.mapKey === k ? "selected" : ""}>${MAP_CATALOG[k].label}</option>`).join("")}
          </select>
        </div>
        <div class="field"><label>Map name (display)</label><input type="text" data-k="mapName" data-i="${i}" value="${esc(m.mapName || MAP_CATALOG[m.mapKey]?.displayName || "")}" /></div>
      </div>
      ...
      <div class="checkbox-field"><input type="checkbox" data-k="showTeamLogo" data-i="${i}" ${m.showTeamLogo ? "checked" : ""} /><label>Show team logo</label></div>
      <div class="field"><label>Team logo URL</label><input type="url" data-k="teamLogoUrl" data-i="${i}" value="${esc(m.teamLogoUrl || "")}" /></div>
      <div class="field"><label>Team logo upload</label><input type="file" accept="image/*" data-upload-logo="${i}" /></div>
      <div class="field"><label>Team initials (winner footer)</label><input type="text" data-k="teamInitials" data-i="${i}" value="${esc(m.teamInitials || "")}" maxlength="4" /></div>
      <div class="checkbox-field"><input type="checkbox" data-k="isWinner" data-i="${i}" ${m.isWinner ? "checked" : ""} /><label>This match is finished (WWCD)</label></div>
...
function wireSingleMatchEditorEvents() {
  ...
  els.matchesContainer.querySelectorAll("[data-k]").forEach((input) => {
    ...
    input.addEventListener(ev, () => {
      ...
      if (k === "isWinner" && input.checked) {
        state.winner.matchIndex = i;
        state.winner.showChickenDinner = true;
        if (els.showChicken) els.showChicken.checked = true;
      ...
      if (k === "isWinner" && !input.checked) {
        state.matches[i].isWinner = false;
        state.matches[i].showWwcd = false;
      }
      void persist();
    });
  });
  ...
  els.matchesContainer.querySelectorAll("[data-upload-logo]").forEach((input) => {
    input.addEventListener("change", () => fileToDataUrl(input.files[0], (url) => {
      const i = Number(input.dataset.uploadLogo);
      state.matches[i].teamLogoUrl = url;
      state.matches[i].showTeamLogo = true;
      if (state.winner.matchIndex === i) state.winner.teamLogoUrl = url;
      void persist();
      renderSingleMatchEditor(i);
    }));
  });
...
function wireEvents() {
  [
    ...
    el?.addEventListener("input", () => {
      ...
      if (el && (el === els.showChicken || el === els.winnerIndex)) {
        readForm();
        const idx = Math.max(0, Math.min(7, Number(state.winner.matchIndex) || 0));
        state.matches.forEach((m, i) => {
          const win = state.winner.showChickenDinner && i === idx;
          m.isWinner = win;
          m.showWwcd = win;
        });
        if (state.winner.showChickenDinner && state.matches[idx]) {
          if (state.winner.teamLogoUrl) state.matches[idx].teamLogoUrl = state.winner.teamLogoUrl;
          if (state.winner.teamInitials) state.matches[idx].teamInitials = state.winner.teamInitials;
        }
        renderMatchTabs();
        renderSingleMatchEditor(selectedMatchIndex);
      }
  ...
  els.bgUpload?.addEventListener("change", async () => {
    ...
    try {
      ...
      const url = await uploadScheduleBackground(file);
      state.background.imageUrl = url;
      state.background.mediaType = probe.isVideo ? "video" : "image";
